...
def print_stats(notas_alumnos, grado_alumnos):
    print("Points average: {:.1f}".format(sum(notas_alumnos) / len(notas_alumnos)))
    passing_grades = sum(1 for grade in grado_alumnos if grade > 0)
    pass_percentage = (passing_grades / len(grado_alumnos)) * 100
